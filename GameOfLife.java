...
public class GameOfLife {
	...
	public static void test2(String fileName) {
		...
		int i=3;
		int j=2;
		System.out.println("The value of the cell in row "+i+" and line "+j+" is: "+board[i][j]);
		System.out.println("The value of the cell next generation :"+cellValue(board,i,j));
	...
	public static int[][] read(String fileName) {
		...
		//filling the array according to the given data
		int j=1;
	...
	public static int[][] evolve(int[][] board) {
		...
		for(int i=1;i<board.length-1;i++)
		{
			...
			{
				board1[i][j]=cellValue(board,i,j);
			}
		}
		return board1;
	}
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		{
			if(count==2 || count==3)//if the cell has 2 or 3 neighbors
			{
				cellvalue=1;//returns that the cell will live
			}
			else//if the cell has less than 2 or more than 3 neighbors 
			{
				cellvalue=0;//returns that the cell will die
			}
		}
		else //if the cell is dead
		{
			if(count==3)//if the cell has exactly 3 neighbors
			{
				cellvalue=1;
			}
			else
